...
#   NJOBS   [8]      parallel workers (set -1 to use all cores)
#   N       [1024]   Sobol base sample N  (evals = N*(groups+2))
#   TMAX    [3.0]    trajectory free-integration window (s)
#   STAGE   [sobol]  'smoke', 'sobol', 'morris', or 'both'
#                    ('smoke' = tiny N=4 end-to-end check, ~30 s, then exit)
#   SOCE    [1]      1 = include SOCE group, 0 = base model only
...
if [ "$STAGE" = "morris" ] || [ "$STAGE" = "both" ]; then
  ...
  "$PY" -m sensitivity_analysis.run_morris \
      --r 20 --trajectory --t-max "$TMAX" $SOCE_FLAG \
...
if [ "$STAGE" = "sobol" ] || [ "$STAGE" = "both" ]; then
  ...
  "$PY" -m sensitivity_analysis.run_sobol \
      --n "$N" --trajectory --t-max "$TMAX" $SOCE_FLAG \
